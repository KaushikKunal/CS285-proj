...
class PGAgent(nn.Module):
    ...
    def update(
        ...
        if self.critic is not None:
            ...
            qval_mean, qval_std = np.mean(q_values), np.std(q_values)
            q_values = (q_values - qval_mean) / (qval_std + 1e-8)
    ...
    def _estimate_advantage(
        ...
        if self.critic is None:
            ...
            values = ptu.to_numpy(self.critic(obs))[..., 0]
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        involve t)!
        """
        gamma_mult = np.power(self.gamma, np.arange(len(rewards)))
        Q = np.sum(rewards * gamma_mult)
        return np.ones(len(rewards)) * Q
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        for i in reversed(range(len(rewards))):
            val = rewards[i] + self.gamma * val
            discounted_reward[i] = val
